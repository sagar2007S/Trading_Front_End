...
const SymbolSearchPanel = ({ onSelectSymbol, onClose }) => {
  ...
  return (
    <div className="flex w-[700px] h-[500px] bg-white rounded-lg shadow-lg border border-gray-200 absolute top-30 right-46">
      {/* Left Sidebar - Categories */}
      <div className="w-48 bg-gray-50 border-r border-gray-200 rounded-l-lg">
        <div className="p-3 border-b border-gray-200">
          <h2 className="text-base font-semibold text-gray-800">Add symbol</h2>
        </div>
          ...
          {categories.map((category) => (
            <button
              ...
              className={`w-full px-3 py-2 flex items-center justify-between hover:bg-blue-50 transition-colors ${
                selectedCategory === category.name ? 'bg-blue-100 border-l-4 border-blue-600' : ''
              }`}
            >
              <span className={`text-sm ${
                selectedCategory === category.name ? 'text-blue-600 font-medium' : 'text-gray-700'
              }`}>
                {category.name}
              </span>
